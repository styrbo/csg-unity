	...
	public class CSGObject : MonoBehaviour
	{
		...
		public void DrawDebug()
		{
			foreach (Face f in faces)
			{
				...
				{
					Debug.DrawLine(f.vertices[i], f.vertices[(i + 1) % f.vertices.Length],
						new Color(1.0f, 0.0f, 1.0f, 1.0f), 20.0f, true);
				}
			}
		}
		...
		public void TransferFacesToMesh()
		{
			...
			foreach (Material m in allMaterials)
			{
				...
				foreach (Face f in faces)
				{
					...
					TextureMapper texMapper = new TextureMapper(f.GetPlane(), globalTexScaleU, globalTexScaleV,
						globalTexOffsetU, globalTexOffsetV, 0.0f);
