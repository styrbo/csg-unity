...
namespace CSG
{
	class TextureMapper
	{
		...
		public TextureMapper(Plane inPlane, float inScaleU, float inScaleV, float inOffsetU, float inOffsetV,
			float inRotation)
		{
			Vector3[] baseAxis = new Vector3[]
			{
				new Vector3(0.0f, 1.0f, 0.0f), new Vector3(1.0f, 0.0f, 0.0f), new Vector3(0.0f, 0.0f, -1.0f), // floor
				new Vector3(0.0f, -1.0f, 0.0f), new Vector3(-1.0f, 0.0f, 0.0f),
			...
			// 
			basis[0] = new Vector3(baseAxis[bestAxis * 3 + 1].x, baseAxis[bestAxis * 3 + 1].y,
				baseAxis[bestAxis * 3 + 1].z);
			basis[1] = new Vector3(baseAxis[bestAxis * 3 + 2].x, baseAxis[bestAxis * 3 + 2].y,
				baseAxis[bestAxis * 3 + 2].z);
		...
		public Vector2 Project(Vector3 inPt)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				float u = rotCos * Vector3Component(baseEntryU, basis[i]) -
				          rotSin * Vector3Component(baseEntryV, basis[i]);
				float v = rotCos * Vector3Component(baseEntryU, basis[i]) +
				          rotSin * Vector3Component(baseEntryV, basis[i]);
				//		rotatedBasis[0]	
